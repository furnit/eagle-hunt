class Employee::EmployeebaseController < ApplicationController
  ...
  def flag_processed id
    ...
    proc = Employee::Processed.find_or_create_by(
      ...
      as_symbol: acu_is?(:admin) ? session[:admin_as_employee]["sym"] : current_user.user_type.symbol
    )
  ...
  def has_processed_before id
    Employee::Processed.where(
      ...
      user_id: current_user.id,
      as_symbol: (acu_is?(:admin) ? session[:admin_as_employee]["sym"] : current_user.user_type.symbol)).empty?
  end
