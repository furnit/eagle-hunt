class Employee::FanisController < Employee::EmployeebaseController
  ...
  def edit
    unless has_processed_before params[:id]
      ...
      # build details
      build_details = @form[:fani].build_details;
      # fetch the details
      @form[:build_details] = @form[:fani].build_details if not build_details.empty?
    end
  end
  ...
  def create
    ...
    build_details_params[:spec].each do |spec_id, sections|
      sections[:section].each do |section_id, params|
        ...
        @form[:build_details] << Admin::Furniture::BuildDetail.new(
    ...
    respond_to do |format|
      if valid.all?
        ...
        # link stuff together
        @form[:build_details].each { |f| Employee::FanisFurnitureBuildDetails.create(employee_fani_id: @form[:fani].id, admin_furniture_build_detail_id: f.id) }
  ...
  def update_field
    ...
    if p
      ...
      if p
        ...
        if p[:admin_furniture_build_detail]
          Admin::Furniture::BuildDetail.update(p[:admin_furniture_build_detail][:id], build_details_params)
  ...
  def set_forms_instance
    ...
    6.times { @form[:build_details] << Admin::Furniture::BuildDetail.new }
  end
  ...
  def build_details_params
    params.require(:admin_furniture_furniture).require(:employee_fani).require(:admin_furniture_build_detail).except(:id).select { |k, v| [:value, :options, :spec].include? k.to_sym }.permit!
  end
