class Admin::FabricsController < Admin::UploaderController
  before_action :set_admin_fabric, only: [:show, :edit, :update, :list_images, :archive]
  ...
  def create
    ...
    respond_to do |format|
      if @admin_fabric.save
        ...
        format.html { redirect_to @admin_fabric, notice: "طرح «<b>#{@admin_fabric.id}</b>» با موفقیت ایجاد شد." }
  ...
  def update
    ...
    respond_to do |format|
      if @admin_fabric.update(admin_fabric_params)
        format.html { redirect_to @admin_fabric, notice: "طرح «<b>#{@admin_fabric.id}</b>» با موفقیت ویرایش شد." }
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to admin_fabrics_url, notice: "طرح «<b>#{@admin_fabric.id}</b>» با موفقیت حذف شد." }
