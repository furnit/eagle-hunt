class Admin::FurnitureFabricColorsController < Admin::AdminbaseController
  def index
    @admin_furniture_fabric_colours = Admin::FurnitureFabricColor.all
  end
  ...
  def destroy
    ...
    item = Admin::FurnitureFabricColor.find(params[:id])
    ...
    Admin::FurnitureFabricColor.all.each.with_index { |e, index| e.id = (index + 1); e.model = new_model; e.save } 
  ...
  def compute
    Thread.new do
      ...
      Admin::FurnitureFabricColor.cluster k, runs: AppConfig.fabric.colours.cluster.runs
      
      Admin::FurnitureFabric.all.each { |f| f.determine_colour }
