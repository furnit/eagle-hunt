class Admin::Selling::Config::PricesController < ApplicationController
  ...
  def create
    ...
    respond_to do |format|
      if @admin_selling_config_price.save
        format.html { redirect_to admin_selling_config_prices_path, notice: mk_notice(@admin_selling_config_price, :id, 'Price', :create) }
        format.json { render json: @admin_selling_config_price, status: :created, location: admin_selling_config_prices_path }
      else
  ...
  def update
    respond_to do |format|
      if @admin_selling_config_price.update(admin_selling_config_price_params)
        ...
        format.json { render json: @admin_selling_config_price, status: :ok, location: admin_selling_config_prices_path }
      else
  ...
  def destroy
    ...
    respond_to do |format|
      format.html { redirect_to admin_selling_config_prices_path, notice: mk_notice(@admin_selling_config_price, :id, 'Price', :destroy) }
  ...
  private
    ...
    def admin_selling_config_price_params
      params.require(:admin_selling_config_price).permit(:admin_furniture_fabric_brand_id, :admin_furniture_paint_color_brand_id, :admin_furniture_wood_type_id)
    end
end
