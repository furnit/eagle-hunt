class Admin::Furniture::Fabric::ColorsController < Admin::AdminbaseController
  def index
    @admin_furniture_fabric_colours = Admin::Furniture::Fabric::Color.all
  end
  
  def update
    color = Admin::Furniture::Fabric::Color.update(params[:id], params.require(:admin_furniture_fabric_color).permit(:name, :comment))
  ...
  def destroy
    ...
    item = Admin::Furniture::Fabric::Color.find(params[:id])
    ...
    Admin::Furniture::Fabric::Color.all.each.with_index { |e, index| e.id = (index + 1); e.model = new_model; e.save } 
